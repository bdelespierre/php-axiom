 ...
 * @subpackage helper
 */
class Axiom_TableRowGroupHelper extends Axiom_BaseHelper {
     ...
     * Filter setter
     * @param array $filter
     * @return Axiom_TableRowGroupHelper
     */
    ...
    /**
     * Works exactly as Axiom_TableRowGroupHelper::before does but
     ...
     *
     * @see Axiom_TableRowGroupHelper::addRow
    ...
    public function addRow ($values, $cell_type = "auto") {
        if (is_scalar($values))
            $values = array($values);
        elseif ($values instanceof Axiom_Model)
            $values = $values->getData();
        elseif (!is_array($values) && !$values instanceof Traversable) {
            throw new InvalidArgumentException(
            	"First parameter is expected to be scalar, array or Axiom_Model, ".get_class($values)." given", 3002
            );
        }
        ...
        $this->appendChild(Axiom_TableRowHelper::export($values, $cell_type));
        return $this;
    }
