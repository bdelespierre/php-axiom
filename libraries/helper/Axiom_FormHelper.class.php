 ...
 * @subpackage helper
 */
class Axiom_FormHelper extends Axiom_BaseHelper {
     ...
     * @see Axiom_FormLineHelper::export
     * @param string $name
     * @param string $display_name = null
     * @param string $type = "text"
     * @param scalar $value = ""
     * @return Axiom_FormHelper
     */
    public function addLine ($name, $display_name = null, $type = "text", $value = "", $class = "") {
        $this->appendChild($this->_form_lines[$name] = Axiom_FormLineHelper::export($name, $display_name, $type, $value, $class));
        return $this;
    }
     ...
     * @see Axiom_FieldsetHelper::export
     * @param strign $legend = ""
     * @return Axiom_FieldsetHelper
     */
    public function addFieldset ($legend = "") {
        return $this->appendChild($this->_fieldsets[] = Axiom_FieldsetHelper::export($legend));
    }
     ...
     * or a Axiom_Model instance.
     * @param mixed $desc
     * @return Axiom_FormHelper
     */
    public function autoFill ($desc) {
        foreach ($this->_children as &$node) {
            if ($node instanceof Axiom_FieldsetHelper) {
                $node->autoFill($desc);
            }
            if ($node instanceof Helper) {
                $name = $node->getName();
                if (is_object($desc) && $desc instanceof Axiom_Model && isset($desc->$name)) {
                    $value = $desc->$name;
                }
