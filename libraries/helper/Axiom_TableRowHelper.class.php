 ...
 * @subpackage helper
 */
class Axiom_TableRowHelper extends Axiom_BaseHelper {
     ...
     * @return Axiom_TableRowHelper
     */
    public function addCells ($values, $cells_type = "data") {
        if (is_scalar($values))
            $values = array($values);
        elseif (is_object($values) && $values instanceof Axiom_Model)
            $values = $values->getData();
        elseif (!is_array($values) && !$values instanceof Traversable)
            throw new InvalidArgumentException("First parameter is expected to be scalar, array or Axiom_Model, ".get_class($values)." given", 2049);
    ...
    public function addCell ($value, $cells_type = "data") {
        $this->appendChild(Axiom_TableDataHelper::export($value, $cells_type));
        return $this;
    }
