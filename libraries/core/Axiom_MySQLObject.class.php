 ...
 * into a Axiom_Model object.
 *
 * Basicaly, this class is just a Axiom_Model class
 * capable of understanding the table
 * structure and to translate it to generic
 * CRUD queries.
 *
 * Due to Axiom_Model's limitation, only tables
 ...
 * extending directly from Axiom_Model.
 *
 ...
 * @subpackage core
 */
class Axiom_MySQLObject extends Axiom_Model {
    ...
    protected function _init ($statement) {
        ...
        return $this->_statements[$statement] = Axiom_Database::prepare($query);
    }
    ...
    protected function _getTableStructure ($table) {
        ...
        if ($stmt = Axiom_Database::query("DESC $table")) {
     ...
     * instance of Axiom_MySQLObject will be generated.
     *
     ...
     * @param array $options
     * @param Axiom_MySQLObject $mysql_obj
     * @return Axiom_PDOStatementIterator
     */
    public static function all ($table, array $search_params = array(), array $options = array(), Axiom_MySQLObject $mysql_obj = null) {
        if (!isset($mysql_obj))
        ...
        Axiom_Log::debug('Query: '. $query);
        
        $stmt = Axiom_Database::prepare($query);
