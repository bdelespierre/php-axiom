...
class Axiom_ViewManager {
    ...
    public static function load ($controller, $action) {
        ...
        Axiom_Log::debug("Loading view: {$__filename}");
        
        if (!$__filename) {
            Axiom_Log::warning("No view defined for {$__section}/{$__view} with format {$__format}");
            return;
        }
        ...
        if (self::$_response->layout()) {
            if ($__layout = self::getLayoutFilePath($__format))
                ...
                Axiom_Log::warning("No such layout {$__layout}");
        }
        else
     ...
     * Add a view path
     * @param string $path
     * @throws Axiom_MissingFileException if $path is not a valid directory
     * @return void
     */
    public static function addPath ($path) {
        if (is_dir($path))
            ...
            throw new Axiom_MissingFileException($path, 2006);
    }
     ...
     * @return void
     */
    public static function setResponse (Axiom_Response &$response) {
    ...
    public static function partial ($section, $view, $format = "html") {
        if (!$__filename = self::getViewFilePath($section, $view, $format)) {
            Axiom_Log::warning("Partial loading failed: no view for {$section}/{$view} with format {$format}");
            return false;
        }
...
/**
 * Non PHP-doc
 * @see Axiom_ViewManager::partial
 */
function partial ($section, $view, $format = "html") {
    return Axiom_ViewManager::partial($section, $view, $format);
}
